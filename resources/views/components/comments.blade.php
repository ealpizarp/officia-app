    ...
    <div class="container">
        <div class="row justify-content-center mt-2">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body">
                        <h4 class="font-bold leading-tight text-gray-700 text-2xl mt-0 mb-4 dark:text-gray-300">User reviews</h4>
                        @foreach ($reviews as $review)
        
                            <article class="py-5">
                                <div class="flex items-center mb-4 space-x-4">
                                    <img class="w-10 h-10 rounded-full hover" src="{{ $review->user->profile_photo ? asset('storage/' . $review->user->profile_photo) : asset('./images/profile_template.png')}}"
                                        alt="">
                                    <div class="space-y-1 font-medium dark:text-gray-300">
                                        <p>{{$review->user->name}} {{$review->user->last_names}}<time datetime="2014-08-16 19:00"
                                                class="block text-sm text-gray-500 dark:text-gray-300">Joined on
                                ...
                                <div class="flex items-center mb-1">
                                    ...
                                    <h3 class="ml-2 text-sm font-semibold text-gray-900 dark:text-gray-300">{{$review->num_stars}} stars</h3>
                                </div>
                                <footer class="mb-5 text-sm text-gray-500 dark:text-gray-300">
                                    <p>Reviewed on <time
                                            datetime="{{ $review->created_at }}">{{ date('F, Y h:i A', strtotime($review->created_at)) }}</time>
                                    </p>
                                </footer>
                                <p class="mb-2 text-gray-700 dark:text-gray-300">{{ $review->body }}
                                </p>
                                {{-- <a href="#"
                                    class="block mb-5 text-sm font-medium text-blue-600 hover:underline dark:text-blue-500">Read
                                    more</a> --}}
                                <aside>
                                    {{-- <p class="mt-1 text-xs text-gray-500 dark:text-gray-300">19 people found this
                                        helpful</p> --}}
                                    {{-- <div
